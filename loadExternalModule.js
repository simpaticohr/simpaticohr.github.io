function renderModuleFrame(container, sectionId, href) {
    const frameId = `frame-${sectionId}`;
    container.innerHTML = `
        <div id="loading-${sectionId}" style="display:flex;justify-content:center;align-items:center;height:300px;background:white;border-radius:12px;">
            <div class="text-center" style="text-align:center;">
                <i class="fas fa-circle-notch fa-spin mb-3" style="font-size:2.5rem;color:var(--primary);margin-bottom:12px;"></i>
                <div style="font-weight:600;color:var(--text-secondary);">Initializing SaaS Engine...</div>
            </div>
        </div>
        <iframe id="${frameId}" src="${href}" 
            style="width:100%; min-height:85vh; border:none; border-radius:12px; background:transparent; display:none;" 
            loading="lazy">
        </iframe>
    `;
    const iframe = document.getElementById(frameId);
    const loader = document.getElementById(`loading-${sectionId}`);
    if (iframe) {
        iframe.onload = () => {
            ...
            try {
                ...
                // Cross-origin restriction (safe fallback: min-height 85vh already set)
            }
        };
        iframe.onerror = () => {
            if (loader) loader.style.display = 'none';
            container.innerHTML = `<div style="padding:40px;text-align:center;color:var(--danger);">Failed to load module.</div>`;
...
async function loadExternalModule(sectionId, href) {
    ...
    // Direct iframe render for file:/// protocol (fetch is blocked by browser sandbox)
    if (window.location.protocol === 'file:') {
        renderModuleFrame(container, sectionId, href);
        return;
    }
    ...
    try {
        ...
        console.warn(`[Module Loader] Fetch injection failed for ${sectionId}, falling back to iframe:`, error);
        try {
            renderModuleFrame(container, sectionId, href);
        } catch (frameErr) {
            console.error("Iframe fallback also failed:", frameErr);
