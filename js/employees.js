function renderTable(list) {
  ...
  tbody.innerHTML = list.map(e => {
    const firstNameStr = typeof e.first_name === 'string' ? e.first_name : String(e.first_name || '').trim();
    const lastNameStr   = typeof e.last_name === 'string' ? e.last_name : String(e.last_name || '').trim();
    const name          = `${firstNameStr} ${lastNameStr}`;
    const initials      = (firstNameStr[0] + lastNameStr[0]) || '?'; // Safe access
    const color         = avatarColor(e.id);
    const dept          = e.departments?.name || '—';
    const mgr           = e.manager && typeof e.manager.first_name === 'string' 
      ? `${e.manager.first_name} ${e.manager.last_name}` 
      : '—';
...
function renderTable(list) {
  ...
  tbody.innerHTML = list.map(e => {
    ...
    const initials= e.first_name && e.last_name ? `${e.first_name[0]}${e.last_name[0]}`.toUpperCase() : 'U';
    const color   = avatarColor(e.id);
    const dept    = e.departments?.name || '—';
