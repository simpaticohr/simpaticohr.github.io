...
async function handleFixRLS(request, env, ctx) {
  const sql = `
    ...
    -- Create consulting_kpi_history table for trend analysis and forecasting
  ...
  `;
  try {
    const res = await sbFetch(env, "POST", "/rest/v1/rpc/execute_sql", { sql: sql }, false, "default");
...
async function handleCreateApplication(request, env, ctx) {
  ...
  // ★ GENERAL APPLICATION AUTO-FALLBACK:
  // If job_id is missing/null/general, attempt to resolve an active open job for the tenant or fallback cleanly
...
async function handleCreateInterview(request, env, ctx) {
  ...
  const payload = {
    ...
    ai_difficulty: body.ai_difficulty || "adaptive",
    round_number: body.round_number || 1,
    parent_interview_id: body.parent_interview_id || null,
    difficulty_auto_escalate: body.difficulty_auto_escalate !== false,
...
async function handleValidateInterview(request, env, ctx) {
  ...
  return apiResponse({
    ...
    interview: {
      ...
      ai_difficulty: interview.ai_difficulty || "adaptive",
