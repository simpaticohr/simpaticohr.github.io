...
const SCHEMAS = {
  ...
  payroll_run: {
    ...
    optional: ['notes','type'],
    rules: {
      period:   v => /^\d{4}-(0[1-9]|1[0-2])(-\d{1,2})?$/.test(v) || 'period must be YYYY-MM',
      pay_date: v => !isNaN(Date.parse(v)) || 'Invalid pay_date',
      type:     v => !v || ['monthly','weekly','bi_weekly','supplemental','bonus','off_cycle'].includes(v) || 'Invalid payroll type',
    }
  },
  performance_review: {
